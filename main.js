 ...
 */
async function takeCompositedScreenshot(targetUrl, viewportWidth, viewportHeight) {
    ...
    try {
        ...
        console.log(`[${validatedTargetUrl}] Fetching background: ${validatedBgImageUrl}`);
        let backgroundImageDataUri;
        try {
            const bgResponse = await fetch(validatedBgImageUrl);
            if (!bgResponse.ok) {
                throw new Error(`Failed background fetch (${validatedBgImageUrl}): ${bgResponse.status}`);
            }
            const contentType = bgResponse.headers.get('content-type') || 'image/jpeg';
            const effectiveContentType = contentType.startsWith('image/') ? contentType : 'image/jpeg';
            const imageBuffer = await bgResponse.arrayBuffer();
            backgroundImageDataUri = `data:${effectiveContentType};base64,${arrayBufferToBase64(imageBuffer)}`;
            console.log(`[${validatedTargetUrl}] Background fetched.`);
        } catch (bgError) {
            console.error(`[${validatedTargetUrl}] Error fetching background:`, bgError);
        ...
        console.log(`[${validatedTargetUrl}] Setting initial viewport: ${viewportWidth}x${viewportHeight}`);
        ...
        await page.evaluateOnNewDocument(() => {
            ...
            const style = document.createElement('style'); style.id='disable-motion-style'; style.append(document.createTextNode(css)); document.documentElement.append(style);
        });

        console.log(`[${validatedTargetUrl}] Navigating to target...`);
        try {
             await page.goto(validatedTargetUrl, { waitUntil: 'networkidle0', timeout: PAGE_LOAD_TIMEOUT });
        } catch (gotoError) {
             console.warn(`[${validatedTargetUrl}] Navigation with networkidle0 failed (may be expected for SPAs): ${gotoError.message}. Trying load...`);
             // Fallback or alternative wait strategy if networkidle0 fails often
             try {
                await page.goto(validatedTargetUrl, { waitUntil: 'load', timeout: PAGE_LOAD_TIMEOUT });
             } catch (loadError) {
                 console.error(`[${validatedTargetUrl}] Navigation failed completely: ${loadError.message}`);
        ...
        const screenshotBase64 = await page.screenshot({ encoding: 'base64', type: 'jpeg', quality: 95 });
        ...
        if (cappedWidth > MAX_WIDTH) {
            ...
            effectiveScreenshotWidth = Math.round(effectiveScreenshotWidth * scaleFactor);
            effectiveScreenshotHeight = Math.round(effectiveScreenshotHeight * scaleFactor);
        }
        if (cappedHeight > MAX_HEIGHT) {
            ...
            effectiveScreenshotWidth = Math.max(1, Math.round(cappedWidth - PADDING * 2 * (cappedWidth / finalWidth)));
            effectiveScreenshotHeight = Math.max(1, Math.round(cappedHeight - PADDING * 2 * (cappedHeight / finalHeight)));
        }

        console.log(`[${validatedTargetUrl}] Final canvas: ${cappedWidth}x${cappedHeight}, Effective screenshot: ${effectiveScreenshotWidth}x${effectiveScreenshotHeight}`);
        ...
        console.log(`[${validatedTargetUrl}] Setting final viewport: ${cappedWidth}x${cappedHeight}`);
        await page.setViewport({ width: cappedWidth, height: cappedHeight, deviceScaleFactor: 1 });

        console.log(`[${validatedTargetUrl}] Setting page content to generated HTML...`);
        await page.setContent(htmlContent, { waitUntil: 'load', timeout: HTML_LOAD_TIMEOUT });
        ...
        // Wait for final image element
         try {
            ...
            await page.waitForSelector('img#screenshot', { visible: true, timeout: FINAL_IMAGE_WAIT_TIMEOUT });
            console.log(`[${validatedTargetUrl}] Final image element visible.`);
        } catch (waitError) {
            console.warn(`[${validatedTargetUrl}] Warning: Waiting for img#screenshot timed out. Taking screenshot anyway.`);
            // await delay(250); // Optional fallback delay
        }

        console.log(`[${validatedTargetUrl}] Taking final screenshot...`);
        // Return the buffer directly
        const finalImageBuffer = await page.screenshot({ type: 'jpeg', quality: 95 });
...
app.get('/screenshot', async (req, res) => {
    ...
    console.log(`Received request for URL: ${validatedUrl}, Size: ${width}x${height}`);

    try {
        const imageBuffer = await takeCompositedScreenshot(validatedUrl, width, height);
        ...
        console.error(`[${validatedUrl}] Failed to process screenshot request:`, error);
        // Avoid sending detailed internal errors to the client in production
        res.status(500).send(`Error generating screenshot: ${error.message || 'An internal error occurred'}`);
    }
});
